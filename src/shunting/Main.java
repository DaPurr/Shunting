...
public class Main {
	...
	public static void main(String[] args) {
		...
		if (feasible) {
			System.out.println("The schedule is feasible, the rerun counter is " + rerunCounter);
			done = true;
		} else {
			System.out.println("The schedule is not feasible, the rerun counter is " + rerunCounter);
			needScheduleAgain = true;
			Set<MaintenanceActivity> tardyJobs = feasibilityCheck.getTardyJobs();
			for (MaintenanceActivity ta : tardyJobs) {
				System.out.println(
						ta.getJob() + " , " + ta.getStartPlatform() + " , " + ta.getStartWasher() + " , Platform: "
								+ ta.getPlatform() + " , Washer: " + ta.getWasher() + " , " + ta.getEndTime());
		...
		while (rerunCounter < maxRerun && !done) {
			if (needScheduleAgain) {
				System.out.println("Scheduling is rerun again, it is a " + rerunCounter
						+ " run. Remaining reruns available are " + (maxRerun - rerunCounter));
				rerunCounter++;
				...
				for (MaintenanceActivity a : activities) {
					System.out.println(
							a.getJob() + " , " + a.getStartPlatform() + " , " + a.getStartWasher() + " , Platform: "
									+ a.getPlatform() + " , Washer: " + a.getWasher() + " , " + a.getEndTime());
				}
				feasibilityCheck = new FeasibilityCheckScheduling(activities);
				feasible = feasibilityCheck.getFeasible();
				if (feasible) {
					System.out.println("The schedule is feasible, the rerunCounter is " + rerunCounter);
					done = true;
					break;
				} else {
					System.out.println("The schedule is not feasible, the rerun counter is " + rerunCounter);
					...
					Set<MaintenanceActivity> tardyJobs = feasibilityCheck.getTardyJobs();
					for (MaintenanceActivity ta : tardyJobs) {
						System.out.println(ta.getJob() + " , " + ta.getStartPlatform() + " , " + ta.getStartWasher()
								+ " , Platform: " + ta.getPlatform() + " , Washer: " + ta.getWasher() + " , "
								+ ta.getEndTime());
						System.out.println(
								"The real deadline is " + (ta.getJob().getMatchBlock().getDepartureTime() - 3));
						MatchBlock m = ta.getJob().getMatchBlock();
						matchBlockTardy.add(m);
						tardiness.put(m, feasibilityCheck.getTardiness(ta) + c);
		...
		//
		// // test for TrainType, all methods work (times not yet taken into
		// account -> 0 returned)
		//
