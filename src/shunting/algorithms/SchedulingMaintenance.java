...
//		jobsWashingMachine = new HashSet <JobWashingMachine>();
		...
		for(MatchBlock tb:tardyJobs) {
			int arrivaltime=tb.getArrivalTime();
			int inspectiontime=tb.getPart1().getInspectionTime();
			int platformtime=tb.getPart1().getPlatformTime();
			int departuretime=tb.getDepartureTime();
			int washingtime=tb.getPart1().getWashingTime();
			
			
			if(tb.getPart1().getPartRepair()||tb.getPart1().getPartCleaning()) {
				if(tb.getPart1().getPartWashing()) {
					Job job = new Job(tb, arrivaltime + 2 + inspectiontime, platformtime, washingtime, departuretime-3-tardiness.get(tb));
					jobs.add(job);
				}

				else {
					Job job = new Job(tb, arrivaltime + 2 + inspectiontime, platformtime, 0, departuretime-3-tardiness.get(tb));
					jobs.add(job);
				}
	...
	public int[] timeRoutine() {
		...
		else System.out.println("Tatiana something is wrong. Minimum of the arrival " +minArrivalPlatform+ " min departure time "+minDeparturePlatform+
				" min arrival washing machine "+minArrivalWashingMachine+" min departureWashingMachine "+minDepartureWashingMachine);
	...
	private void platformArrival() {
		...
		for (Job j: timeArrivalPlatform.keySet()) {
			if(time == timeArrivalPlatform.get(j) & !queuePlatform.contains(j)) {
				queuePlatform.add(j);
				//System.out.println("The arrival to the platform queue of job "+j+" at time "+time);
			}
		}
